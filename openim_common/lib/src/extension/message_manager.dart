...
extension MessageManagerExt on MessageManager {
  ...
  /// 表格卡片消息（下注清单/账单等）。[table] 结构见 TableCardData：
  /// { title, subtitle?, titleBg?, columns:[{label,flex?,align?}], rows:[{cells:[{text,color?,bold?}]}], summary?:[{label,value,color?}] }
  Future<Message> createTableCardMessage({required Map<String, dynamic> table}) =>
      createCustomMessage(
        data: json.encode({
          "customType": CustomMessageType.tableCard,
          "data": table,
        }),
        extension: '',
        description: '',
...
class CustomMessageType {
  ...
  /// 表格卡片（下注清单/账单等，前端原生渲染彩色表格，取代图片）
  static const tableCard = 920;
}
