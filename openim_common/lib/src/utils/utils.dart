...
class IMUtils {
        ...
        case MessageType.custom:
          ...
          switch (customType) {
            ...
            case CustomMessageType.groupDisbanded:
              ...
              break;
            case CustomMessageType.tableCard:
              content = (customData is Map && customData['title'] != null) ? '${customData['title']}' : '[表格]';
              break;
            default:
              content = '[${StrRes.unsupportedMessage}]';
              break;
        ...
        case MessageType.custom:
          {
            ...
            switch (customType) {
              ...
              case CustomMessageType.meeting:
                ...
                return map['data'];
              case CustomMessageType.tableCard:
                map['data']['viewType'] = CustomMessageType.tableCard;
                return map['data'];
              case CustomMessageType.deletedByFriend:
              case CustomMessageType.blockedByFriend:
              case CustomMessageType.removedFromGroup:
  ...
  static void parseClickEvent(
    ...
    if (message.contentType == MessageType.picture || message.contentType == MessageType.video) {
      var mediaMessages =
          messageList.where((element) => element.contentType == MessageType.picture || element.contentType == MessageType.video).toList();
      var currentIndex = mediaMessages.indexWhere((element) => element.clientMsgID == message.clientMsgID);
      if (currentIndex == -1) {
  ...
  static bool isValidPassword(String password) => RegExp(
        ...
        // r'^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d\S]{6,20}$', // 原来的规则：必须包含字母和数字
