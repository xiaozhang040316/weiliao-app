...
class ChatPicturePreview extends StatelessWidget {
  ...
  Widget _networkGestureImage(MediaSource source) {
    // 视频：直接展示播放器(封面用缩略图)，不走图片手势逻辑
    ...
    // 图片：直接以手势(可缩放)模式加载“大图”本身，缩略图仅作加载中的占位。
    // 旧实现是在缩略图上做手势、加载完成后又覆盖一张“非手势”大图，导致点开无法放大。
      ...
      loadStateChanged: (ExtendedImageState state) {
        switch (state.extendedImageLoadState) {
          case LoadState.loading:
            {
              // 大图加载期间先显示缩略图，避免白屏；无缩略图则显示进度圈
              if (source.thumbnail.isNotEmpty) {
                return ExtendedImage.network(
                  source.thumbnail,
                  fit: BoxFit.contain,
                  handleLoadingProgress: true,
