...
class _ChatPictureViewState extends State<ChatPictureView> {
  ...
  void didUpdateWidget(covariant ChatPictureView oldWidget) {
    ...
    // 列表滚动会复用同一个 State；消息本身也可能更新(本地图上传完成拿到URL、
    // 收到的图片信息刷新)。若不重算，会停留在旧图/破损图，表现为“串图、需刷新才正常”。
    final o = oldWidget.message;
    final n = widget.message;
    final changed = o.clientMsgID != n.clientMsgID ||
        o.pictureElem?.sourcePath != n.pictureElem?.sourcePath ||
        o.pictureElem?.bigPicture?.url != n.pictureElem?.bigPicture?.url ||
        o.pictureElem?.snapshotPicture?.url != n.pictureElem?.snapshotPicture?.url;
    if (changed) {
      _child = null;
      _init();
    }
  ...
  Widget _buildPathPicture({required String path}) => Stack(
        children: [
          ImageUtil.fileImage(
            ...
            width: _trulyWidth,
            fit: BoxFit.fitWidth,
            // 本地原图可能是数千万像素，必须降采样解码，否则发图/滚动明显卡顿
            lowMemory: true,
          ),
          // Image(
