...
class UpgradeManger {
  ...
  autoCheckVersionUpgrade() async {
    ...
    // 检查上次检查更新的时间，避免频繁检查
    final lastCheckTime = DataSp.getLastUpdateCheckTime() ?? 0;
    final now = DateTime.now().millisecondsSinceEpoch;
    const checkInterval = 24 * 60 * 60 * 1000; // 24小时检查一次
    
    if (now - lastCheckTime < checkInterval) {
      Logger.print('距离上次检查更新时间不足24小时，跳过检查');
      return;
    }
    ...
    try {
      ...
      // 记录检查时间
      DataSp.putLastUpdateCheckTime(now);
      ...
      Logger.print('自动检查更新失败: $e');
      // 检查更新失败不应该影响用户体验，只记录日志
    }
  }
