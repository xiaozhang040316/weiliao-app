...
class GroupSetupLogic extends GetxController {
  ...
  final isLotteryEnabled = false.obs;
  ...
  void onInit() {
    ...
    final lgid = conversationInfo.value.groupID;
    if (lgid != null && lgid.isNotEmpty) {
      isLotteryEnabled.value = DataSp.isGroupLotteryEnabled(lgid);
    }
  ...
  void searchChatHistory() => AppNavigator.startSearchChatContent(
  ...
  /// 走势图开关（本地按群记住，并通知聊天页显示/隐藏）
  void toggleLottery() {
    final gid = conversationInfo.value.groupID;
    if (gid == null || gid.isEmpty) return;
    final next = !isLotteryEnabled.value;
    DataSp.putGroupLotteryEnabled(gid, next);
    isLotteryEnabled.value = next;
    chatLogic.setLotteryEnabled(next);
  }
}
