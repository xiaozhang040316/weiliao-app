...
class ConversationLogic extends GetxController {
  ...
  /// 同步/拉取消息期间为 true：用遮罩+刷新动画挡住会话列表，禁止用户进群操作，防止误操作扯皮。
  ...
  void onInit() {
    ...
    imLogic.imSdkStatusSubject.listen((value) {
      ...
      _updateSyncMask(value);
    });
    // 被踢出群 / 退群：全局删除该群的本地会话与全部历史消息，
    // 确保离开群后再也看不到该群聊天记录（重新入群按新的入群时间只看入群后消息）。
    imLogic.joinedGroupDeletedSubject.listen((info) => _purgeGroupConversation(info.groupID));
    imLogic.memberDeletedSubject.listen((info) {
      if (info.userID == OpenIM.iMManager.userID) _purgeGroupConversation(info.groupID);
    });
    super.onInit();
  }

  /// 根据 SDK 状态切换同步遮罩：开始/进行中同步→显示；同步结束/失败/连接成功→隐藏。
  void _updateSyncMask(IMSdkStatus s) {
    final syncing = s == IMSdkStatus.syncStart || s == IMSdkStatus.synchronizing;
    _syncMaskTimer?.cancel();
    if (syncing) {
      showSyncMask.value = true;
      // 安全超时：同步异常长时间未结束时，最多遮罩 20 秒后自动放行，避免把用户永久挡在外面。
      _syncMaskTimer = Timer(const Duration(seconds: 20), () => showSyncMask.value = false);
    } else {
  ...
  /// 删除某群的本地会话与全部消息（被踢/退群时调用）。
  Future<void> _purgeGroupConversation(String? groupID) async {
    if (groupID == null || groupID.isEmpty) return;
    try {
      // 优先用已存在的会话对象拿 conversationID，拿不到再按 OpenIM 群会话规则(sg_ 前缀)兜底。
      String? cid;
      for (final c in list) {
      ...
      cid ??= 'sg_$groupID';
      await OpenIM.iMManager.conversationManager.deleteConversationAndDeleteAllMsg(conversationID: cid);
      list.removeWhere((c) => c.groupID == groupID);
      _sortConversationList();
    } catch (e) {
      Logger.print('purge group conversation failed: $e');
    }
  }
  ...
  void toChat({
    ...
    // 同步消息期间禁止进入聊天，避免消息未加载完成时误操作。
    if (showSyncMask.value) {
      IMViews.showToast('正在同步消息，请稍候…');
      return;
    }
    // 获取会话信息，若不存在则创建
